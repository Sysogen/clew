...
commit whose subject is the pull request title and whose body is your commit
messages. Three things follow:
...
- **Your individual commit messages do not have to follow the convention.** CI
  ...
  the merge. Commit however helps you work; a maintainer can edit the squash
...
`Release` waits for CI to finish on that commit, since `Tag` starts it as soon
as the change lands, and refuses to publish unless CI succeeded, the tag
matches the manifest version, and the tag is an ancestor of `main`. It
